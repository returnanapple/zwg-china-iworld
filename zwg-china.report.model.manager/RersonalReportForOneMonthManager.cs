...
namespace zwg_china.model.manager
{
    ...
    /// 单月个人统计的管理者对象
    /// </summary>
    public class RersonalReportForOneMonthManager : ManagerBase<IModelToDbContextOfReport, RersonalReportForOneMonthManager.Actions, RersonalReportForOneMonth>
    {
        #region 构造方法
        ...
        public RersonalReportForOneMonthManager(IModelToDbContextOfReport db)
            : base(db)
        {
        ...
        public static void CreateReportOnTimeline(InfoOfSendOnManagerService info)
        {
            ...
            RersonalReportForOneMonthManager manager = new RersonalReportForOneMonthManager(db);
            ...
            db.Authors.Where(x => x.Status == UserStatus.正常)
                ...
                .ForEach(user =>
                {
                    ...
                    #endregion
                    RersonalReportForOneMonth record = new RersonalReportForOneMonth(user, date, money, timesOfLogin, amountOfBets, rebate
                        , bonus, commission, reward, dividend, recharge, withdrawal);
                    manager.OnExecuting(Actions.Create, record);
                    db.RersonalReportForOneMonths.Add(record);
